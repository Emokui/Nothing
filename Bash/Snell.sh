...
check_environment() {
  ...
  case "$os_id" in
    debian|ubuntu) ;;
    *)
      echo -e "${RED}仅支持 Debian 和 Ubuntu${PLAIN}"
      return 1
      ;;
  esac

  if ! command -v systemctl >/dev/null 2>&1; then
    echo -e "${RED}未检测到 systemd${PLAIN}"
    return 1
  fi
}
...
pause_any_key_and_clear() {
  press_any_key_to_continue "${1:-按任意键继续...}"
  clear
}
...
main() {
  ...
  snell_get_arch >/dev/null || exit 1
  snell_menu
}
