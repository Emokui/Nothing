...
reinstall_debian() {
    ...
    install_runner=$(mktemp) || {
        echo -e "${RED}无法创建临时运行脚本${PLAIN}"
        return
    }

    sed '/^# ====== 主程序入口 ======/,$d' "$install_script" >"$install_runner"
    cat >>"$install_runner" <<'EOF'
check_sys
[[ "$EUID" -ne '0' ]] && echo -e "${Error} 请使用 root 权限运行此脚本" && exit 1
[[ -z "${release}" ]] && echo -e "${Error} 暂不支持当前系统" && exit 1
first_job
installnet_main -d "${REINSTALL_DEBIAN_VERSION}" -v 64 -a -p "${REINSTALL_ROOT_PASSWORD}" -port "${REINSTALL_SSH_PORT}"
EOF
