...
issue_acme_cert() {
    ...
    if [ ! -f "$ACME_ACCOUNT_CONF" ] || ! grep -q "ACCOUNT_EMAIL=" "$ACME_ACCOUNT_CONF"; then
        read -p "$(echo -e "${YELLOW}請輸入Email,回车随机生成）: ${PLAIN}")" email
        if [ -z "$email" ]; then
    ...
    read -p "$(echo -e "${YELLOW}請輸入你的域名${PLAIN}（解析好本机IP）: ")" domain
    if [[ -z "$domain" ]]; then
...
cert_menu() {
    while true; do
        ...
        echo "#############################################"
        echo ""
        echo -e " ${GREEN}1.${PLAIN} 生成自簽名證書"
        echo -e " ${GREEN}2.${PLAIN} 申請域名證書"
        echo -e " ${GREEN}0.${PLAIN} 返回世界线"
        echo ""
...
select_cert_for_hysteria() {
    ...
    echo -e "${GREEN}1.${PLAIN} 自簽證書"
    echo -e "${GREEN}2.${PLAIN} 域名證書"
    echo -e "${GREEN}3.${PLAIN} 手动输入证书路徑"
    echo -e "${GREEN}0.${PLAIN} 返回世界线"
...
EOF
    ...
    while true; do
        ...
        echo -e "${GREEN}6.${PLAIN} 刪除 Hysteria 服務"
        ...
        case "$ACTION" in
            ...
            7)
                ...
                read -p "$(echo -e "${YELLOW}請輸入認證密碼 (原值: ${old_password}): ${PLAIN}")" auth_password
                if [ -z "$auth_password" ]; then
